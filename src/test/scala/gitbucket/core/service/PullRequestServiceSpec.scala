...
class PullRequestServiceSpec
    ...
    |return None if all pull request is closed""".stripMargin.trim) {
            withTestDB { implicit se =>
                ...
                val r1 = swap(generateNewPullRequest(
                  "user1/repo1/master2",
                  "user1/repo1/head1",
                  loginUser = "root"
                ))
                val r2 = swap(generateNewPullRequest(
                  "user1/repo1/master",
                  "user1/repo1/head1",
                  loginUser = "root"
                ))
                val r3 = swap(generateNewPullRequest(
                  "user1/repo1/master4",
                  "user1/repo1/head1",
                  loginUser = "root"
                ))
                ...
                assert(
                  Seq(r1, r2)
                      .contains(getPullRequestFromBranch("user1", "repo1", "head1", "master").get)
                )
