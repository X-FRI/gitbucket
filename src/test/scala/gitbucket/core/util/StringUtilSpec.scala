...
class StringUtilSpec extends AnyFunSpec {
    ...
    describe("escapeHtml") {
        it("should escape &, <, > and \"") {
            assert(
              StringUtil.escapeHtml("<a href=\"/test\">a & b</a>") ==
                  "&lt;a href=&quot;/test&quot;&gt;a &amp; b&lt;/a&gt;"
            )
        }
    }
    ...
    describe("extractGlobalIssueId") {
        it("should extract '#xxx' and return extracted id") {
            assert(
              StringUtil.extractGlobalIssueId("(refs #123)").toSeq ==
                  List((None, None, Some("123")))
            )
        }
        it("should extract 'owner/repository#xxx' and return extracted owner, repository and id") {
            assert(
              StringUtil.extractGlobalIssueId("(refs root/test#123)").toSeq ==
                  List((Some("root"), Some("test"), Some("123")))
            )
        }
        it("should return Nil from message which does not contain #xxx") {
    ...
    describe("extractCloseId") {
        ...
        it("should extract 'close #x, #y, #z' and return extracted multi id") {
            assert(
              StringUtil.extractCloseId("(close #1, #2, #3, wip #4, close #5)").toSeq ==
                  Seq("1", "2", "3", "5")
            )
        }
    }
    ...
    describe("getRepositoryViewerUrl") {
        ...
        it("should convert GitBucket repository url") {
            assert(
              StringUtil.getRepositoryViewerUrl(
        ...
        it("should convert GitHub repository url") {
            assert(
              StringUtil.getRepositoryViewerUrl("https://github.com/root/gitbucket.git", baseUrl) ==
                  "https://github.com/root/gitbucket"
            )
            assert(
              StringUtil
                  .getRepositoryViewerUrl("https://root@github.com/root/gitbucket.git", baseUrl) ==
                  "https://github.com/root/gitbucket"
            )
        }
        it("should convert BitBucket repository url") {
            assert(
              StringUtil
                  .getRepositoryViewerUrl("https://bitbucket.org/root/gitbucket.git", baseUrl) ==
        ...
        it("should convert GitLab repository url") {
            assert(
              StringUtil.getRepositoryViewerUrl("https://gitlab.com/root/gitbucket.git", baseUrl) ==
                  "https://gitlab.com/root/gitbucket"
            )
            assert(
              StringUtil
                  .getRepositoryViewerUrl("https://root@gitlab.com/root/gitbucket.git", baseUrl) ==
                  "https://gitlab.com/root/gitbucket"
            )
        }
    }
