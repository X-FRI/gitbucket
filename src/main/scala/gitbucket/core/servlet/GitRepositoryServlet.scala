...
class GitRepositoryServlet extends GitServlet with SystemSettingsService {
    ...
    override def service(req: HttpServletRequest, res: HttpServletResponse): Unit = {
        ...
        if (index >= 0 && (agent == null || agent.toLowerCase.indexOf("git") < 0)) {
          ...
          req.getRequestURI.endsWith("/info/lfs/objects/batch")
        ) { withLockRepository(req) { serviceGitLfsBatchAPI(req, res) } }
        else {
            // response for git client
            withLockRepository(req) { super.service(req, res) }
        }
    }
   ...
   */
    protected def serviceGitLfsBatchAPI(req: HttpServletRequest, res: HttpServletResponse): Unit = {
        ...
        settings.baseUrl match {
            case None => { throw new IllegalStateException("lfs.server_url is not configured.") }
            case Some(baseUrl) => {
                val index = req.getRequestURI.indexOf(".git")
                if (index >= 0) {
                    req.getRequestURI.substring(0, index).split("/").reverse match {
                        case Array(repository, owner, _*) =>
                            ...
                            val batchResponse = batchRequest.operation match {
                                case "upload" => GitLfs.BatchUploadResponse(
                                      "basic",
                                      batchRequest.objects.map { requestObject =>
                                          GitLfs.BatchResponseObject(
                                            ...
                                            requestObject.size,
                                            true,
                                            GitLfs.Actions(upload =
                                                Some(GitLfs.Action(
                                                  href = baseUrl + "/git-lfs/" + owner + "/" +
                                                      repository + "/" + requestObject.oid,
                                                  header = Map(
                                                    "Authorization" -> StringUtil.encodeBlowfish(
                                                      s"$timeout ${requestObject.oid}"
                                                    )
                                                  ),
                                ...
                                case "download" => GitLfs.BatchUploadResponse(
                                      "basic",
                                      batchRequest.objects.map { requestObject =>
                                          GitLfs.BatchResponseObject(
                                            requestObject.oid,
...
class GitBucketRepositoryResolver extends RepositoryResolver[HttpServletRequest] {
    ...
    override def open(req: HttpServletRequest, name: String): Repository = {
        ...
        }.getOrElse { new FileRepository(new File(Directory.RepositoryHome, name)) }
    }
...
class GitBucketReceivePackFactory
    ...
    override def create(request: HttpServletRequest, db: Repository): ReceivePack = {
        ...
        if (PluginRegistry().getRepositoryRouting(request.gitRepositoryPath).isEmpty) {
            ...
            if (repository.endsWith(".wiki")) {
                receivePack.setPostReceiveHook(new WikiCommitHook(
                  owner,
                  repository.stripSuffix(".wiki"),
                  pusher,
                  baseUrl,
                  sshUrl
                ))
...
class CommitLogHook(
    ...
    def onPreReceive(
        ...
        commands: java.util.Collection[ReceiveCommand]
    ): Unit = {
            ...
            try {
                commands.asScala.foreach { command =>
                    ...
                    PluginRegistry().getReceiveHooks.flatMap(
                      _.preReceive(owner, repository, receivePack, command, pusher, false)
                ...
                Using.resource(Git.open(Directory.getRepositoryDir(owner, repository))) { git =>
                    commands.asScala.foreach { command =>
                        ...
                        if (commits.size < 100) {
                            newCommitIds = commits.takeWhile { commit => !existCommit(git, commit) }
                                .map(_.id)
                        } else {
                            val allCommits = JGitUtil.getAllCommitIds(git)
                            newCommitIds = commits.collect {
                                case commit if !allCommits.contains(commit.id) => commit.id
                            }
                        }
                    }
                }
            ...
            try {
                Using.resource(Git.open(Directory.getRepositoryDir(owner, repository))) { git =>
                    ...
                    commands.asScala.foreach { command =>
                        ...
                        if (
                          JGitUtil.isEmpty(git) && commits.nonEmpty &&
                          branchName != repositoryInfo.repository.defaultBranch
                        ) {
                            saveRepositoryDefaultBranch(owner, repository, branchName)
                            // Change repository HEAD
                            Using
                                .resource(Git.open(Directory.getRepositoryDir(owner, repository))) {
                                    git =>
                                        git.getRepository.updateRef(Constants.HEAD, true)
                                            .link(Constants.R_HEADS + branchName)
                                }
                        }

                        // Retrieve all issue count in the repository
                        val issueCount = countIssue(
                        ...
                        val newCommits = commits.collect {
                            case commit
                                if newCommitIds.contains(commit.id) &&
                                    !pushedIds.contains(commit.id) =>
                                if (issueCount > 0) {
                                    pushedIds.add(commit.id)
                                    createIssueComment(owner, repository, commit)
                                    // close issues
                                    if (
                                      refName(1) == "heads" && branchName == defaultBranch &&
                                        ...
                                        getAccountByUserName(pusher).foreach { pusherAccount =>
                                            closeIssuesFromMessage(
                                              ...
                                              repository
                                                ...
                                                getIssue(owner, repository, issueId.toString)
                                                    .foreach { issue =>
                                                        callIssuesWebHook(
                                                          "closed",
                                                          repositoryInfo,
                                                        ...
                                                        PluginRegistry().getIssueHooks.foreach(
                                                          _.closedByCommitComment(
                                                            issue,
                                                            repositoryInfo,
                                                            commit.fullMessage,
                                                            pusherAccount
                        ...
                        // set PR as merged
                        val pulls =
                            getPullRequestsByBranch(owner, repository, branchName, Some(false))
                        pulls.foreach { pull =>
                            if (commits.exists { c => c.id == pull.commitIdTo }) {
                        ...
                        if (refName(1) == "heads") {
                            command.getType match {
                                case ReceiveCommand.Type.CREATE =>
                                    val createBranchInfo =
                                        CreateBranchInfo(owner, repository, pusher, branchName)
                                    recordActivity(createBranchInfo)
                                case ReceiveCommand.Type.UPDATE =>
                                    val pushInfo =
                                        PushInfo(owner, repository, pusher, branchName, newCommits)
                                    recordActivity(pushInfo)
                                case ReceiveCommand.Type.DELETE =>
                                    val deleteBranchInfo =
                                        DeleteBranchInfo(owner, repository, pusher, branchName)
                            ...
                            command.getType match {
                                case ReceiveCommand.Type.CREATE =>
                                    val createTagInfo =
                                        CreateTagInfo(owner, repository, pusher, branchName)
                                    recordActivity(createTagInfo)
                                case ReceiveCommand.Type.DELETE =>
                                    val deleteTagInfo =
                                        DeleteTagInfo(owner, repository, pusher, branchName)
                        ...
                        if (refName(1) == "heads") {
                            command.getType match {
                                case ReceiveCommand.Type.CREATE |
                                    ReceiveCommand.Type.UPDATE | ReceiveCommand.Type
                                        .UPDATE_NONFASTFORWARD => getAccountByUserName(pusher)
                                        .foreach { pusherAccount =>
                                            updatePullRequests(
                                              owner,
                                              repository,
                                              branchName,
                        ...
                        PluginRegistry().getReceiveHooks.foreach(
                          _.postReceive(owner, repository, receivePack, command, pusher, false)
                        )
                    }
                }
...
class WikiCommitHook(
    ...
    override def onPostReceive(
        receivePack: ReceivePack,
        commands: util.Collection[ReceiveCommand]
    ): Unit = {
        val settings = loadSystemSettings()
            ...
            try {
                commands.asScala.headOption.foreach { command =>
                    ...
                    commitIds.foreach { case (oldCommitId, newCommitId) =>
                        val commits = Using
                            .resource(Git.open(Directory.getWikiRepositoryDir(owner, repository))) {
                                git =>
                                    JGitUtil.getCommitLog(git, oldCommitId, newCommitId)
                                        .flatMap { commit =>
                                            ...
                                            diffs.collect {
                                                case diff
                                                    if diff.newPath.toLowerCase.endsWith(".md") =>
                                                    ...
                                                    buildWikiRecord(
                                                      ...
                                                      commit,
                                                      fileName
                                                    ).foreach(recordActivity)
                                                    (action, fileName, commit.id)
                                            }
                                        }
                            }

                        val pages = commits.groupBy { case (_, fileName, _) => fileName }
    ...
    private[this] def buildWikiRecord(
        ...
        action match {
            ...
            case "edited" =>
                Some(EditWikiPageInfo(owner, repo, commit.committerName, pageName, commit.id))
...
object GitLfs {
    ...
    case class BatchRequestObject(oid: String, size: Long)

    case class BatchUploadResponse(transfer: String, objects: Seq[BatchResponseObject])
    ...
    case class Actions(download: Option[Action] = None, upload: Option[Action] = None)

    case class Action(href: String, header: Map[String, String] = Map.empty, expires_at: Date)
