...
trait RepositoryViewerControllerBase extends ControllerBase {
    ...
    case class TagForm(commitId: String, tagName: String, message: Option[String])
   ...
   */
    get("/:owner/:repository") {
        ...
        if (RepositoryCreationService.isCreating(owner, repository)) {
            ...
            params.get("go-get") match {
                case Some("1") => getRepository(owner, repository)
                        .map(gitbucket.core.html.goget(_)) getOrElse NotFound()
    ...
    ajaxGet("/:owner/:repository/creating") {
        ...
        Serialization.write(Map(
          ...
          "error" ->
              (if (creating) None
               else RepositoryCreationService.getCreationError(owner, repository))
        ))
    }
   ...
   */
    ...
    post("/:owner/:repository/upload", uploadForm)(
      writableUsersOnly { (form, repository) =>
          ...
          context.withLoginAccount { loginAccount =>
              ...
              val newFiles = files.map { file =>
                  file.copy(name =
                      if (form.path.length == 0) file.name else s"${form.path}/${file.name}"
                  )
              }

              if (form.newBranch) {
                  val newBranchName =
                      createNewBranchForPullRequest(repository, form.branch, loginAccount)
                  _commit(newBranchName, newFiles, loginAccount) match {
                      case Right(objectId) =>
                          val issueId = createIssueAndPullRequest(
                            repository,
                            form.branch,
                            newBranchName,
                            form.commit,
                  ...
                  _commit(form.branch, newFiles, loginAccount) match {
                      case Right(_) =>
                          if (form.path.length == 0) {
                              redirect(
                                s"/${repository.owner}/${repository.name}/tree/${encodeRefName(form.branch)}"
                              )
                          } else {
                              redirect(s"/${repository.owner}/${repository
                                      .name}/tree/${encodeRefName(form.branch)}/${encodeRefName(form.path)}")
                          }
                      case Left(error) => Forbidden(gitbucket.core.html.error(error))
                  }
              }
            ...
            Using.resource(Git.open(getRepositoryDir(repository.owner, repository.name))) { git =>
                ...
                getPathObjectId(git, path, revCommit).map { objectId =>
                    ...
                    html.editor(
                      ...
                      content = JGitUtil.getContentInfo(
                        git,
                        path,
                        objectId,
                        repository.repository.options.safeMode
                      ),
                      protectedBranch = protectedBranch,
                      commit = revCommit.getName,
        ...
        Using.resource(Git.open(getRepositoryDir(repository.owner, repository.name))) { git =>
            ...
            getPathObjectId(git, path, revCommit).map { objectId =>
                ...
                html.delete(
                  ...
                  fileName = paths.last,
                  content = JGitUtil
                      .getContentInfo(git, path, objectId, repository.repository.options.safeMode),
                  commit = revCommit.getName
                )
            } getOrElse NotFound()
        }
    ...
    post("/:owner/:repository/create", editorForm)(
      writableUsersOnly { (form, repository) =>
              ...
              if (form.newBranch) {
                  ...
                  _commit(form.branch, loginAccount) match {
                      case Right(_) =>
                          if (form.path.length == 0) {
                              redirect(s"/${repository.owner}/${repository
                                      .name}/blob/${encodeRefName(form.branch)}/${urlEncode(form.newFileName)}")
                          } else {
                              redirect(
                                s"/${repository.owner}/${repository.name}/blob/${encodeRefName(
                                      form.branch
                                    )}/${encodeRefName(form.path)}/${urlEncode(form.newFileName)}"
                              )
                          }
                      case Left(error) => Forbidden(gitbucket.core.html.error(error))
                  }
      ...
      writableUsersOnly { (form, repository) =>
          def _commit(branchName: String, loginAccount: Account): Either[String, ObjectId] = {
              commitFile(
                ...
                charset = form.charset,
                message =
                    if (form.oldFileName.contains(form.newFileName)) {
                        form.message.getOrElse(s"Update ${form.newFileName}")
                    } else {
                        form.message
                            .getOrElse(s"Rename ${form.oldFileName.get} to ${form.newFileName}")
                    },
                commit = form.commit,
                loginAccount = loginAccount,
                settings = context.settings
   ...
   */
            ...
            getPathObjectId(git, path, revCommit).map { objectId =>
                if (raw) {
                    ...
                    html.blob(
                      ...
                      latestCommit = new JGitUtil.CommitInfo(
    ...
    private def getSyntaxHighlighterTheme()(implicit context: Context): String = {
        context.loginAccount match {
            case Some(account) => getAccountPreference(account.userName) match {
   ...
   */
    ajaxGet("/:owner/:repository/get-blame/*")(referrersOnly { repository =>
        ...
        contentType = formats("json")
        Using.resource(Git.open(getRepositoryDir(repository.owner, repository.name))) { git =>
            val last = git.log.add(git.getRepository.resolve(id)).addPath(path).setMaxCount(1).call
                .iterator.next.name
            Serialization.write(Map(
              ...
              "blame" -> JGitUtil.getBlame(git, id, path).map { blame =>
                  Map(
                    "id" -> blame.id,
                    "author" -> view.helpers.user(blame.authorName, blame.authorEmailAddress)
                        .toString,
                    "avatar" ->
                        view.helpers.avatarLink(blame.authorName, 32, blame.authorEmailAddress)
                            .toString,
   ...
   */
    ...
    post("/:owner/:repository/commit/:id/comment/new", commentForm)(readableUsersOnly {
        (form, repository) =>
            context.withLoginAccount { loginAccount =>
                val id = params("id")
                createCommitComment(
    ...
    ajaxPost("/:owner/:repository/commit/:id/comment/_data/new", commentForm)(readableUsersOnly {
        (form, repository) =>
            context.withLoginAccount { loginAccount =>
                val id = params("id")
                ...
                val comment =
                    getCommitComment(repository.owner, repository.name, commentId.toString).get
                helper.html.commitcomment(
                  comment,
                  hasDeveloperRole(repository.owner, repository.name, context.loginAccount),
                  repository
                )
            ...
            getCommitComment(repository.owner, repository.name, params("id")) map { x =>
                if (isEditable(x.userName, x.repositoryName, x.commentedUserName, loginAccount)) {
                    params.get("dataType") collect {
                        case t if t == "html" =>
                            html.editcomment(x.content, x.commentId, repository)
                    } getOrElse {
                        contentType = formats("json")
              ...
              getCommitComment(repository.owner, repository.name, params("id")).map { comment =>
                  if (
                    isEditable(
                      ...
                      redirect(
                        s"/${repository.owner}/${repository.name}/commit_comments/_data/${comment.commentId}"
                      )
                  } else Unauthorized()
              } getOrElse NotFound()
          }
   ...
   */
    get("/:owner/:repository/branches")(referrersOnly { repository =>
        ...
        val branches = Using
            .resource(Git.open(getRepositoryDir(repository.owner, repository.name))) { git =>
                JGitUtil.getBranches(
                  git = git,
                  defaultBranch = repository.repository.defaultBranch,
                  origin = repository.repository.originUserName.isEmpty
                ).sortBy(branch => (branch.mergeInfo.isEmpty, branch.commitTime)).map(branch =>
                    (
                      branch,
   ...
   */
    private def fileList(
        ...
        Using.resource(Git.open(getRepositoryDir(repository.owner, repository.name))) { git =>
            if (JGitUtil.isEmpty(git)) {
                ...
                JGitUtil.getDefaultBranch(git, repository, revstr)
                    .map { case (objectId, revision) =>
                        ...
                        val readme = files // files should be sorted alphabetically.
                            .find { file =>
                                !file.isDirectory &&
                                RepositoryService.readmeFiles.contains(file.name.toLowerCase)
                            }.map { file =>
                        ...
                        html.files(
                          ...
                          getCommitStatusWithSummary(
                            repository.owner,
                            repository.name,
                            lastModifiedCommit.getName
                          ),
                          commitCount,
                          files,
                          readme,
    ...
    private def archiveRepository(
        ...
        def archive[A <: ArchiveEntry](
            revision: String,
            archiveFormat: String,
            archive: ArchiveOutputStream[A]
        )(entryCreator: (String, Long, java.util.Date, Int) => A): Unit = {
            Using.resource(Git.open(getRepositoryDir(repository.owner, repository.name))) { git =>
                val oid = git.getRepository.resolve(revision)
                val commit = JGitUtil.getRevCommitFromId(git, oid)
                val date = commit.getCommitterIdent.getWhen
                val sha1 = oid.getName()
                val repositorySuffix = (if (sha1.startsWith(revision)) sha1 else revision)
                    .replace('/', '-')
                            ...
                            if (treeWalk.getFileMode != FileMode.GITLINK) {
                                val entryPath =
                                    if (path.isEmpty) baseName + "/" + treeWalk.getPathString
                                        ...
                                        treeWalk.getPathString.substring(path.length)
                                ...
                                JGitUtil.openFile(
                                  ...
                                  treeWalk.getPathString
                                    ...
                                    val size = Using
                                        .resource(new FileOutputStream(tempFile)) { out =>
                                            IOUtils.copy(
        ...
        val suffix = path.split("/").lastOption
            .collect { case x if x.length > 0 => "-" + x.replace('/', '_') }.getOrElse("")
        val zipRe = """(.+)\.zip$""".r
            ...
            LockUtil.lock(s"${owner}/${repository}") {
                Using.resource(Git.open(getRepositoryDir(owner, repository))) { git =>
                    ...
                    if (headTip.getName != value) {
                        Some(
                          "Someone pushed new commits before you. Please reload this page and re-apply your changes."
                        )
                    } else { None }
                }
            }
