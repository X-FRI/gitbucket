...
//  case class SearchForm(query: String, owner: String, repository: String)
    ...
    get("/") {
        context.loginAccount.map { account =>
            val visibleOwnerSet: Set[String] = Set(account.userName) ++
                getGroupsByUserName(account.userName)
            if (!isNewsFeedEnabled) { redirect("/dashboard/repos") }
            else {
                ...
                gitbucket.core.html.index(
                  ...
                  showBannerToCreatePersonalAccessToken = hasAccountFederation(account.userName) &&
                      !hasAccessToken(account.userName),
    ...
    post("/signin", signinForm) { form =>
        authenticate(context.settings, form.userName, form.password) match {
            case Some(account) => flash.get(Keys.Flash.Redirect) match {
                    case Some(redirectUrl: String) =>
                        signin(account, redirectUrl + form.hash.getOrElse(""))
    ...
    post("/signin/oidc") {
        context.settings.oidc.map { oidc =>
            ...
            val authenticationRequest =
                createOIDCAuthenticationRequest(oidc.issuer, oidc.clientID, redirectURI)
   ...
   */
        ...
        context.settings.oidc.map { oidc =>
            ...
            session.get(Keys.Session.OidcAuthContext) match {
                case Some(context: OidcAuthContext) =>
                    authenticate(params.toMap, redirectURI, context.state, context.nonce, oidc)
                        .map { case (jwt, account) =>
                            session.setAttribute(
                              Keys.Session.OidcSessionContext,
                              OidcSessionContext(jwt)
                            )
    ...
    get("/signout") {
        context.settings.oidc.foreach { oidc =>
            session.get(Keys.Session.OidcSessionContext)
                .foreach { case context: OidcSessionContext =>
                    val redirectURI = new URI(baseUrl)
                    val authenticationRequest = createOIDLogoutRequest(
                      oidc.issuer,
                      oidc.clientID,
                      redirectURI,
                      context.token
        ...
        session.invalidate()
        if (isDevFeatureEnabled(DevFeatures.KeepSession)) { deleteLoginAccountFromLocalFile() }
        redirect("/")
    }
    ...
    post("/sidebar-collapse") {
        if (params("collapse") == "true") { session.setAttribute("sidebar-collapse", "true") }
        else { session.setAttribute("sidebar-collapse", null) }
        Ok()
    }
   ...
   */
    private def signin(account: Account, redirectUrl: String = "/") = {
        ...
        if (isDevFeatureEnabled(DevFeatures.KeepSession)) { saveLoginAccountToLocalFile(account) }
        updateLastLoginDate(account.userName)

        if (LDAPUtil.isDummyMailAddress(account)) { redirect("/" + account.userName + "/_edit") }

        if (redirectUrl.stripSuffix("/") == request.getContextPath) { redirect("/") }
   ...
   */
    get("/_user/proposals")(usersOnly {
        ...
        org.json4s.jackson.Serialization.write(Map(
          "options" ->
              (getAllUsers(includeRemoved = false).withFilter { t =>
                  (user, group) match {
                      case (true, true)   => true
                  ...
                  Map(
                    "label" -> s"${avatar(t.userName, 16)}<b>@${StringUtil
                            .escapeHtml(StringUtil.cutTail(t.userName, 25, "..."))}</b> ${StringUtil
   ...
   */
    ...
    get("/:owner/:repository/search")(referrersOnly { repository =>
        ...
        target.toLowerCase match {
            case "issues" => gitbucket.core.search.html.issues(
                  if (query.nonEmpty)
                      searchIssues(repository.owner, repository.name, query, pullRequest = false)
                  else Nil,
                  pullRequest = false,
                  query,
                  page,
            ...
            case "pulls" => gitbucket.core.search.html.issues(
                  if (query.nonEmpty)
                      searchIssues(repository.owner, repository.name, query, pullRequest = true)
                  else Nil,
                  pullRequest = true,
                  query,
                  page,
            ...
            case "wiki" => gitbucket.core.search.html.wiki(
                  if (query.nonEmpty) searchWikiPages(repository.owner, repository.name, query)
                  else Nil,
                  query,
                  page,
                  repository
            ...
            case _ => gitbucket.core.search.html.code(
                  if (query.nonEmpty) searchFiles(repository.owner, repository.name, query)
                  else Nil,
                  query,
                  page,
                  repository
    ...
    get("/search") {
        ...
        val visibleRepositories = getVisibleRepositories(
        ...
        val repositories = {
            ...
            } else { visibleRepositories }
        }.filter { repository =>
            repository.name.toLowerCase.indexOf(query) >= 0 ||
            repository.owner.toLowerCase.indexOf(query) >= 0
        }
